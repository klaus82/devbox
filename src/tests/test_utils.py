...
class TestFindDevcontainerConfig:
    ...
    def test_find_devcontainer_config_found(self, mock_getcwd, mock_join, mock_isfile, mock_isdir):
        """Test finding devcontainer.json when it exists"""
        mock_isdir.side_effect = lambda path: path == "/home/user/project/.devcontainer"
        mock_isfile.side_effect = lambda path: path == "/home/user/project/.devcontainer/devcontainer.json"
    ...
    @patch("os.path.join", side_effect=lambda *args: "/".join(args))
    @patch("os.getcwd", return_value="/home/user/project")
    def test_find_devcontainer_config_not_found(self, mock_getcwd, mock_join, mock_isfile, mock_isdir):
